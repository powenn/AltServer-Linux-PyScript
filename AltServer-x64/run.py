...
if subprocess.run('stat AltServerDaemon | grep -- "-rw-r--r--"',shell=True) != "" :
    subprocess.run("chmod +x AltServerDaemon",shell=True)
...
subprocess.run('./AltServerDaemon &> /dev/null &',shell=True)
...
while RunScript==0 :
    ...
    if option == 'd' : # Restart-Daemon-mode
        subprocess.run('killall AltServerDaemon',shell=True)
